        ...
        <div className="fixed inset-0 z-40 bg-black/40 backdrop-blur-sm flex items-end sm:items-center justify-center">
          <div className="w-full sm:max-w-md bg-card rounded-t-3xl sm:rounded-3xl p-6 pb-8 border border-border max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <Field label="Lönedag (dag i månaden)">
                <input
                  type="number"
                  min={1}
                  max={28}
                  value={local.payday}
                  onChange={(e) =>
                    setLocal({ ...local, payday: Math.max(1, Math.min(28, +e.target.value || 27)) })
                  }
                  className="w-24 bg-muted rounded-xl px-3 py-2 tabular-nums outline-none focus:ring-2 focus:ring-accent"
                />
              </Field>

              <Field label="Total budget per löningscykel (kr)">
                <input
                  type="number"
                  value={local.monthly}
              ...
              <Field label="Övrigt per löningscykel (kr)">
                <input
                  type="number"
                  value={local.other}
