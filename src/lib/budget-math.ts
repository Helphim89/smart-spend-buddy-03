...
const WEEKDAY_DAYS_PER_WEEK = 4; // mån–tors
const WEEKEND_DAYS_PER_WEEK = 3; // fre–sön

/* --------------------------- löne-cykel --------------------------- */

/** Lör → fre (-1d), Sön → mån (+1d) */
function adjustPayday(d: Date): Date {
  const day = d.getDay();
...
/** [start, end] (inkl) för cykeln som innehåller ref. Cykeln startar vid lönedagen (helg-justerad). */
export function cycleRange(ref: Date, payday: number): [Date, Date] {
  const pay = Math.min(Math.max(1, Math.round(payday)), 28);
  const y = ref.getFullYear();
  const m = ref.getMonth();
  const candidates = [
    adjustPayday(new Date(y, m - 1, pay)),
    adjustPayday(new Date(y, m, pay)),
    adjustPayday(new Date(y, m + 1, pay)),
    adjustPayday(new Date(y, m + 2, pay)),
  ].map(startOfDay);
  const t = startOfDay(ref).getTime();
  for (let i = 0; i < candidates.length - 1; i++) {
    const s = candidates[i];
    const e = endOfDay(addDays(candidates[i + 1], -1));
    if (t >= s.getTime() && t <= e.getTime()) return [s, e];
  }
  const s = candidates[1];
  const e = endOfDay(addDays(candidates[2], -1));
  return [s, e];
}
...
export function weeksInCycle(
  ...
  while (cursor.getTime() <= cEnd.getTime()) {
    ...
    if (clipped) {
      ...
      const weekdayBudget = Math.round((settings.weekday * wd) / WEEKDAY_DAYS_PER_WEEK);
      const weekendBudget = Math.round((settings.weekend * we) / WEEKEND_DAYS_PER_WEEK);
...
export function computeSnapshot(
  ...
  if (weekClip) {
    ...
    weekdayBudget = Math.round((settings.weekday * weekdayDays) / WEEKDAY_DAYS_PER_WEEK);
    weekendBudget = Math.round((settings.weekend * weekendDays) / WEEKEND_DAYS_PER_WEEK);
  ...
  // Prognos: ÅTERSTÅENDE planerad pro-rata-budget (vardag + helg per vecka) + återstående övrigt
